...
**2026-08-18 — Root-caused a mobile scroll freeze to two bugs in Frappe core, not this app**

Multi-round investigation into a persistent "page freezes, no vertical scroll at all" complaint on `opportunity_analytics_report.js`, using a temporary on-page diagnostic overlay (`logStep()`, a "Probe now" dimension/style snapshot button, global `error`/`unhandledrejection` listeners — all still present, gated to be removable once this is fully confirmed fixed):

- Ruled out, in order, with live evidence each time: `.main-section` DOM-readiness timing at cold load (fixed with a `whenScrollElReady()` rAF-poll regardless, since it was a real minor race even though not the freeze's cause); a CSS/JS `regionSelector` mismatch between the injected `touch-action: pan-x` scope and `attach_axis_lock_scroll()`'s targets (was already correctly scoped from an earlier round — added explicit cross-reference comments at both the CSS rule and the JS call site so they can't drift apart silently again); overlapping/racing `frappe.call`s from rapid filter changes (added a 250ms debounce + request-token staleness guard as a genuine data-consistency improvement regardless, but confirmed via the overlay log this was never actually happening).
- **Actual root cause**: `document.body.style.overflow` getting stuck at `"hidden"`, confirmed via the probe tool's exact matching values. Traced to two independent, confirmed mechanisms in **Frappe core**, not this app: (1) `frappe.ui.Dialog`'s `hide_scrollbar()` (`frappe/public/js/frappe/ui/dialog.js`), tied to Bootstrap's `shown.bs.modal`/`hide.bs.modal` events on every Dialog instance, stuck if a hide transition doesn't fire; (2) `frappe.ui.Page`'s mobile sidebar-toggle button (CSS-hidden above 992px, i.e. mobile-only, present in every desk page's header) calling `sidebar.set_height()` unconditionally on every tap, with no restoring call anywhere in that file's open/close/toggle logic. Neither is fixable at its source from this app (both are vendored framework files shared by every dialog and every desk page).
- Verified this page's own two `frappe.call`s (`get_filter_options`, `get_dashboard_data`) never used `freeze: true` and never defaulted to it either (checked directly against `frappe/public/js/frappe/request.js`) — added it explicitly anyway for clarity, though it changes nothing functionally. Also confirmed the existing loading indicator was already a plain in-place `$body` HTML swap with no `position:fixed`/body-touching of any kind — polished it with a small CSS spinner regardless.
- Defensive mitigation (since the actual bug lives in core): a guard that clears `body.style.overflow` back whenever it's wrongly stuck at `"hidden"` with no dialog legitimately open (`window.cur_dialog`/`frappe.ui.open_dialogs`/`.modal-backdrop` all checked to avoid ever touching a real open dialog's intentional scroll-lock) — run via both a `MutationObserver` on `body`'s `style`/`class` attributes and a redundant 1-second interval poll for the page's lifetime, applied to both the Analytics Report page and (precautionarily, gated to only act while on that route) the Opportunity Dashboard bundle.
- Not yet confirmed fixed on-device as of this entry — still pending an extended, multi-minute real-phone session with periodic checks throughout, not just at load.
